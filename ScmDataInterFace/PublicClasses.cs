...
namespace ScmDataInterFace
{
    ...
    public sealed class Miscellaneous
    {
        ...
        public static string RunExe(string ProccessName, string par)
        {
            ...
            try
            {
                string path = Application.StartupPath + ProccessName;// @"D:\项目文件\webTrans接口服务\website_client\WindowsFormsApplication3\WindowsFormsApplication3\bin\Debug\WindowsFormsApplication3.exe";
    ...
    public sealed class Project
    {
        ...
        public static string scmPath = string.Empty;
        public static string zpKey = string.Empty;
        public static int commandTimeOut = 36000;
    }
    #endregion
            ...
            lock (lockHelper)
            {
                ...
                try
                {
                    ...
                    if (ls_commands.Length == 4)
                    {
                        ...
                        {
                            TaskManager task = new TaskManager(ls_commands[0], schedule_name);
                            bResult = task.exec();
                            //bResult = TaskManager.exec(schedule_name, ls_commands[0]);
        ...
        public static string execute_job_NoThread(string schedule_name, object as_command)
        {
            ...
            try
            {
                ...
                if (ls_commands.Length == 4)
                {
                    ...
                    {
                        TaskManager task = new TaskManager(ls_commands[0], schedule_name);
                        bool bResult = task.exec();
                        //bool bResult = TaskManager.exec(schedule_name, ls_commands[0]);
                        if (bResult)   //任务执行成功后再执行最后处理
                        {
                            Project.TaskSuccessSum++;
                            if (ls_commands[1] != "")
    ...
    public class WSHelper
    {
        ...
        /// < returns>< /returns> 
        public static string InvokewebTrans(string url, string classname, string methodname, object[] args, ref DataTable dt)
        {
            string sResult = string.Empty;
            string @namespace = "EnterpriseServerBase.webTrans.DynamicWebCalling";
